...
const Page = () => {
    ...
    const others = ['Figma', 'Responsive Web Design', 'Performance', 'Optimization', 'Sofware Testing']
    ...
    const educationMap: {
        ...
        gap: 81.2,
        relevantKnowledge: ['Web and mobile development', 'database management', 'data structures', 'object-oriented programming', 'cybersecurity principles', 'cryptography', 'high-performance computing', 'operating systems', 'network design and implementation', 'algorithmic complexity', 'and applied development of computing projects.']
    }]
    ...
    const experienceMap: {
        ...
        description: 'During my time at the organization, I strengthened my expertise in the MERN stack through hands-on development and continuous learning, guided by mentorship from experienced professionals. I also developed skills in Linux server administration and containerization, allowing me to deliver more scalable, reliable, and production-ready solutions.',
        institute: 'CNS - IPICYT'
    },
    {
        name: 'Intern',
        date: 'October 2023 - November 2024',
        description: 'I orchestrated and developed the frontend infraestructure, design and functionality for the proyect LNC-2023-124: "Formación de profesionales en el uso y gestión de herramientas tecnológicas de supercómputo y centro de datos para abordar problemas de impacto social"',
        institute: 'CNS - IPICYT'
    }]
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] grid-rows-[auto] sm:grid-rows-1 gap-x-8 p-12 sm:p-16">
                ...
                <div className="grid row-start-2 sm:row-start-1 gap-y-2 max-h-[90vh] p-4">
                    ...
                    <div>
                        ...
                        <ol className="list-none ml-1 text-sm">
                            {technologies.map((s, i) => <li className={`inline-block sm:block ${i < technologies.length - 1 ? "after:content-[',']" : ""} sm:after:content-['']`} key={i}>{s}</li>)}
                        </ol>
                    </div>
                    <div>
                        <h2 className={`${classesTitle}`}>Others: </h2>
                        <ol className="list-none ml-1 text-sm">
                            {others.map((s, i) => <li className={`inline-block sm:block ${i < others.length - 1 ? "after:content-[',']" : ""} sm:after:content-['']`} key={i}>{s}</li>)}
                        </ol>
                    </div>
                </div>
                {/*Right */}
                <div className="grid grid-cols-2 gap-8 sm:gap-y-4">
                    <div className="col-span-2">
                        ...
                        <p>Focused on performance, maintainability, and user experience, I am committed to continuously learning the tools and practices needed to build reliable and scalable web solutions.</p>
                    </div>
                    <h1 className={`${classesTitle} col-span-2`}>Education:</h1>
                    {educationMap.map((e, i) => (
                        <div key={i} className="col-span-2">
                            <div className="flex flex-col sm:flex-row sm:justify-between">
                                <p><b className="text-primary-500">{e.name}</b> - GPA: {e.gap} / 100</p>
                    ...
                    {experienceMap.map((e, i) => (
                        <div key={i} className="col-span-2">
                            <div className="flex flex-col sm:flex-row sm:justify-between mb-2">
                                <p><b className="text-primary-500">{e.institute}</b> - {e.name}</p>
                                <p className="text-gray-400">{e.date}</p>
                            </div>
                            <p>{e.description}</p>
                        </div>
